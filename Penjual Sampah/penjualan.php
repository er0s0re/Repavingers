...
$query_kebutuhan = "SELECT id_kebutuhan, jenis_sampah, jumlah_sampah FROM kebutuhan_sampah WHERE status_kebutuhan = 'disetujui'";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Query untuk mendapatkan jenis sampah berdasarkan id_kebutuhan
    $query_jenis_sampah = "SELECT jenis_sampah, jumlah_sampah FROM kebutuhan_sampah WHERE id_kebutuhan = '$id_kebutuhan'";
    $result_jenis_sampah = mysqli_query($koneksi, $query_jenis_sampah);

    // Ambil jenis sampah dan jumlah sampah dari hasil query
    if ($row_jenis_sampah = mysqli_fetch_assoc($result_jenis_sampah)) {
        $jenis_sampah = $row_jenis_sampah['jenis_sampah'];
        $jumlah_sampah_kebutuhan = $row_jenis_sampah['jumlah_sampah'];
    } else {
        $jenis_sampah = "Tidak ada jenis sampah yang tersedia"; // Ganti dengan pesan sesuai kebutuhan
        $jumlah_sampah_kebutuhan = 0;
    }

    // Tentukan lokasi penyimpanan file
    $uploadDir = "../image/bukti_sampah/";

    // Upload gambar jika dipilih
    $fotoSampah = "";
    if (!empty($_FILES['foto_sampah']['name'])) {
        $timestamp = time();
        $extension = pathinfo($_FILES['foto_sampah']['name'], PATHINFO_EXTENSION);
        $newName = $timestamp . 'Sampah.' . $extension;

        $fotoSampah = $uploadDir . $newName;

        if (move_uploaded_file($_FILES['foto_sampah']['tmp_name'], $fotoSampah)) {
    ...
    if ($_FILES['foto_sampah']['type'] != 'image/jpeg' && $_FILES['foto_sampah']['type'] != 'image/png') {
        $error = "Hanya file gambar JPG atau PNG yang diizinkan.";
    }
    ...
    if ($error == "") {
        ...
        mysqli_stmt_bind_param($stmt, "ssssss", $id_penjual, $id_kebutuhan, $jumlah_sampah, $satuan_sampah, $tanggal_penjualan, $newName);

        if (mysqli_stmt_execute($stmt)) {
            // Pindahkan file ke lokasi penyimpanan
            move_uploaded_file($_FILES['foto_sampah']['tmp_name'], $uploadDir . $newName);
...
<head>
    ...
    <script>
    function getKebutuhanSampah(id_kebutuhan) {
        // Buat objek XMLHttpRequest
        var xhr = new XMLHttpRequest();

        // Tentukan URL untuk mengirim permintaan
        var url = "get_kebutuhan_sampah.php";

        // Tentukan method untuk mengirim permintaan
        xhr.open("POST", url, true);

        // Tentukan header untuk mengirim permintaan
        xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");

        // Tentukan data yang akan dikirim
        var data = "id_kebutuhan=" + id_kebutuhan;
        ...
        xhr.onload = function() {
            ...
            // Parsing hasil menjadi objek JSON
            var data = JSON.parse(result);

            // Jika permintaan berhasil
            if (data.status == "success") {
                // Isi inputan kebutuhan sampah dengan hasil dari permintaan
                document.querySelector("input[name=jumlah_sampah]").value = data.kebutuhan_sampah;
            } else {
...
<body class="g-sidenav-show  bg-gray-100">
    ...
    <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
        ...
        <nav class="navbar navbar-main navbar-expand-lg mx-5 px-0 shadow-none rounded" id="navbarBlur"
            ...
            <div class="container-fluid py-1 px-2">
                <nav aria-label="breadcrumb">
                    ...
                    <h6 class="font-weight-bold mb-0">Sales</h6>
                </nav>
                    ...
                    <div class="card border shadow-xs mb-4">
                        <div class="card-header border-bottom pb-0">
                            <div class="d-sm-flex align-items-center mb-3">
                                <div>
                                    <h6 class="font-weight-semibold text-lg mb-0">sales form</h6>
                                    <p class="text-sm mb-sm-0">Sales form to sell your junk</p>
                                </div>
                            </div>
                        </div>
                                                ...
                                                <td>
                                                    <form action="" method="post" enctype="multipart/form-data">
                                                        <div class="d-flex px-2">
                                                            <div class="my-auto">
                                                                <h6 class="mb-0 text-sm" for="id_kebutuhan">Select waste type</h6>
                                                            </div>
                                                            <select name="id_kebutuhan"
                                                                class="form-control form-control-sm ms-2" required>
                                                                <?php
                                // Menampilkan opsi jenis sampah berdasarkan id_kebutuhan
                                while ($row_kebutuhan = mysqli_fetch_assoc($result_kebutuhan)) {
                                    echo '<option value="' . $row_kebutuhan['id_kebutuhan'] . '" data-jumlah-sampah="' . $row_kebutuhan['jumlah_sampah'] . '">' . $row_kebutuhan['jenis_sampah'] . '</option>';
                                }
                                ?>
                                                            </select>
                                                        </div>
                                                ...
                                                <td>
                                                    <div class="d-flex px-2">
                                                        <div class="my-auto">
                                                            <h6 class="mb-0 text-sm">Waste requirement</h6>
                                                        </div>
                                                        <input type="number" id="jumlah_sampah_kebutuhan"
                                                            name="jumlah_sampah"
                                                            class="form-control form-control-sm ms-2"
                                                            placeholder="kebutuhan sampah" readonly
                                                            title="Dalam Kilogram"
                                                ...
                                                <td>
                                                    <div class="d-flex px-2">
                                                        <div class="my-auto">
                                                            <h6 class="mb-0 text-sm">Waste amount</h6>
                                                        </div>
                                                        <input type="number" name="jumlah_sampah"
                                                            class="form-control form-control-sm ms-2"
                                                            placeholder="Your waste amount">
                                                    </div>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td>
                                                    <div class="d-flex px-2">
                                                        <div class="my-auto">
                                                            <h6 class="mb-0 text-sm">Waste weight unit</h6>
                                                        </div>
                                                        <select name="satuan_sampah"
                                                            class="form-control form-control-sm ms-2">
                                                ...
                                                <td>
                                                    <div class="d-flex px-2">
                                                        <div class="my-auto">
                                                            <h6 class="mb-0 text-sm">Trash photo</h6>
                                                        </div>
                                                        <input type="file" name="foto_sampah"
                                                            class="form-control form-control-sm ms-2" accept="image/*"
                                                            required>
                                                    </div>
                                                </td>
                                                <td>
                                                    <div class="d-flex px-2">
                                                        <div class="my-auto">
                                                            <h6 class="mb-0 text-sm">Date of sale</h6>
                                                        </div>
        ...
        // Fungsi untuk mengisi otomatis jumlah sampah kebutuhan
        function updateJumlahSampah() {
            // Dapatkan elemen dropdown jenis sampah
            var dropdown = document.getElementsByName('id_kebutuhan')[0];

            // Dapatkan nilai jumlah sampah dari atribut data-jumlah-sampah opsi yang dipilih
            var jumlahSampah = dropdown.options[dropdown.selectedIndex].getAttribute('data-jumlah-sampah');

            // Isi nilai jumlah sampah ke dalam input jumlah sampah kebutuhan
            document.getElementById('jumlah_sampah_kebutuhan').value = jumlahSampah;

            // Dapatkan elemen dropdown satuan sampah
            var satuanDropdown = document.getElementsByName('satuan_sampah')[0];

            // Tampilkan alert jika satuan yang dipilih adalah "kilogram"
            if (satuanDropdown.value === 'kg') {
                alert('Perhatian: Satuan "Kilogram" dipilih. Harap pastikan jumlah sampah sesuai dengan satuan "Kilogram".');
            }
        }

        // Panggil fungsi updateJumlahSampah saat dropdown jenis sampah berubah
        document.getElementsByName('id_kebutuhan')[0].addEventListener('change', updateJumlahSampah);
        
        // Panggil fungsi updateJumlahSampah saat halaman dimuat (untuk mengisi otomatis jika nilai default dipilih)
        window.onload = updateJumlahSampah;
    </script>
