...
// Ambil informasi pabrik berdasarkan id_pabrik dari sesi
$id_pabrik = isset($_SESSION['user_id']) ? $_SESSION['user_id'] : null;
$queryPabrik = "SELECT * FROM pabrik_paving WHERE id_pabrik = $id_pabrik";
$resultPabrik = mysqli_query($koneksi, $queryPabrik);
$rowPabrik = mysqli_fetch_assoc($resultPabrik);
?>
...
<body class="g-sidenav-show  bg-gray-100">
                    ...
                    <div class="card border shadow-xs mb-4">
                        <div class="card-header border-bottom pb-0">
                            <div class="d-sm-flex align-items-center mb-3">
                                <div>
                                    <h6 class="font-weight-semibold text-lg mb-0">Factory Information</h6>
                                    <p class="text-sm mb-sm-0">Factory information on waste requirements</p>
                                </div>
                            </div>
                        </div>
    ...
    <tbody>
        <?php
        // Ambil data laporan penjualan sampah berdasarkan id_pabrik
        $queryLaporan = "SELECT * FROM laporan_penjualan_sampah WHERE id_pabrik = $id_pabrik";
        $resultLaporan = mysqli_query($koneksi, $queryLaporan);

        // Tampilkan data laporan penjualan sampah dalam bentuk tabel
        while ($rowLaporan = mysqli_fetch_assoc($resultLaporan)) {
            echo '<tr>';
            echo '<td>' . $rowLaporan['tanggal_laporan'] . '</td>';
            echo '<td>' . $rowLaporan['total_penjualan'] . '</td>';
            echo '<td>' . $rowLaporan['satuan_sampah'] . '</td>';
            echo '</tr>';
        }
        ?>
    </tbody>
